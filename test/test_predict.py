...
def test_predict_aux():
    ...
    movie_id = np.array([1, 2, 3, 5, 6, 8, 14, 15])

    # Define true output
    output_true = np.array([5, 2, 3, 8, 6, 14, 15])

    # Compute test result
    output_test = predict_aux(distance_list, movie_id)
...
def test_predict_aux_unhappy():
    # Define input lists
    distance_list = np.array([1, 0.43, 0.38, 0.59, 0.21, 0.33, 0.12, 0.02])
    movie_id = np.array([1, 2, 3, 5, 6, 8, 14])

    with pytest.raises(IndexError):
        predict_aux(distance_list, movie_id)
...
def test_predict_matrix():
    ...
    # Compute test result
    output_test = predict_matrix(corr, movie_id)
...
def test_predict_df():
    ...
    # Compute test result
    df_test = predict_df(pred_matrix, movie_id, top_n = 3)
...
def test_predict_df_nonarray():
    ...
    with pytest.raises(TypeError):
        predict_df(pred_matrix, movie_id, top_n = 3)
